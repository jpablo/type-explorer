...
import org.jpablo.typeexplorer.ui.app.components.tabs.inheritanceTab.svgGroupElement.{ClusterElement, LinkElement, NamespaceElement, SvgGroupElement}
import org.jpablo.typeexplorer.ui.app.components.tabs.inheritanceTab.svgGroupElement.path
import org.scalajs.dom
import org.scalajs.dom.HTMLDivElement


private def CanvasContainer(inheritanceSvgDiagram: Signal[InheritanceSvgDiagram], state: InheritanceTabState) =
  div(cls := "h-full overflow-auto border-t border-slate-300 p-1 row-start-2 row-end-3",
    ...
    child <-- inheritanceSvgDiagram.map: diagram =>
      val selection = state.canvasSelectionR.now()
