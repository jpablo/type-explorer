...
def TabsArea(
  ...
  List(
    NavTabs(
      cls := "mt-2 -mb-px",
      inheritance.NavItem,
      appState.appConfigs.signal.map(_.advancedMode).childWhenTrue(semanticDB.NavItem),
    ...
    TabContent(
      ...
      appState.appConfigs.signal.map(_.advancedMode).childWhenTrue(semanticDB.Pane(semanticDBTabContent))
    )
  )
