...
object InheritanceTab:
  ...
  private def CanvasContainer(
    ...
    div(
      cls := "h-full overflow-auto border-t border-slate-300 p-1 row-start-2 row-end-3",
      ...
      child <-- inheritanceSvgDiagram.map: diagram =>
        ...
        inheritanceTabState.canvasSelectionR.update(
          _ -- (selection -- diagram.elementSymbols)
        )
        diagram.toLaminar
      ,
      onClick.preventDefault.compose(
        _.withCurrentValueOf(inheritanceSvgDiagram)
      ) -->
  ...
  private def SelectionSidebar(
    ...
    div(
      cls := "row-start-1 row-end-3 border-l border-slate-300 col-start-4 col-end-5",
      ul(
        cls := "menu menu-compact",
        li(cls := "menu-title", span("selection")),
        li(
  ...
  private def handleSvgClick(
    ...
    val selectedElement: Option[SvgGroupElement] =
      ev.target
        ...
        .map(e =>
          NamespaceElement.from(e) orElse ClusterElement.from(
            e
          ) orElse LinkElement.from(e)
        )
