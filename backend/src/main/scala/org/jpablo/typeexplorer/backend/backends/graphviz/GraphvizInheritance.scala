...
object GraphvizInheritance:
  ...
  private def renderTree(
      ...
      projectSettings: ProjectSettings,
      symbolOptions:   Map[models.GraphSymbol, Option[SymbolOptions]]
  ): Tree[models.Namespace] => (LinkSource & LinkTarget) =
    case Tree.Branch(label, path, children) =>
      val clusterName = path.mkString("/")
      graph(clusterName).cluster.graphAttr
        .`with`(
          Label
            .html(label)
  ...
  private def renderNamespace(
    ...
    val fields =
      if showFields then
        if ns.methods.isEmpty then None
        else
          Some(
            tr(
              td(
                table(
                  emptyStyle.copy(border = 1, cellPadding = 3),
                  for
                    m <- ns.methods
                    if !projectSettings.hiddenFields.contains(m.displayName)
                  yield tr(
                    td(cPadding := 0, align := "LEFT", fontGV(monoFontFace, m.displayName))
                  )
                )
              )
            )
